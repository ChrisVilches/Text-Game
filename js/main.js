...
function showMsgOptions() {
    ...
    if (!isLastPage() || !hasOptions()) {
        ...
        for (var i = 0; i < currMsg.opt.length; i++) {
            var button = "<button onclick='setBooleans(" + i + "); getNextMsg(\"" + currMsg.next[i] + "\");'>" + currMsg.opt[i] + "</button>";
